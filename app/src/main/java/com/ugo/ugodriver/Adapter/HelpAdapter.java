...
import com.ugo.ugodriver.HelpScreen;
...
public class HelpAdapter extends RecyclerView.Adapter<HelpAdapter.ProductHolder>  {
    ...
    public HelpAdapter(ArrayList<BookingModel> bookModel) {
        bookingModel = new ArrayList<>(bookModel);

    }

    @Override
    public HelpAdapter.ProductHolder onCreateViewHolder(ViewGroup viewGroup, int viewType) {
        View view ;
        context = viewGroup.getContext();
        view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.history_row, viewGroup, false);
        return new HelpAdapter.ProductHolder(view);
    }

    @Override
    public void onBindViewHolder(final HelpAdapter.ProductHolder holder, final int i) {
        holder.position = i;
    ...
    private void moveToDetails(int pos){
        Intent i = new Intent(context , HelpScreen.class);
